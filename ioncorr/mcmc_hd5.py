...
infodata    -> information about the data (NHI, z, etc.. used to define the priors) in a dictionary
               {NHI:logNHI,eNHI:errlogNHI,hiflag:hiflag,z:redshift,errz:errorredshift,name:outputname}
...
optim       -> Initialise with optimisation rather than optimising the likelihood for starting ball
               local: find the max likelihood given the intial guess at grid centre (DISABLED)
               global: search for global max in likelihood (DISABLED)
...
Update history:
	-Trystyn Berg (15 August 2019):
		-Rewrote load_history to read hdf5 model grid files. Note that the HDF5 model grid
		must have an parameters/axistag extension with an ordered list/array of 
		'redshift','column','metallicity', and 'density' corresponding to the order of the
		dimensions for each array of the column density grid. Also note that code assumes
		grid is in linear space.
		-Modified plotinfo hdf5 writing to properly deal with strings/arrays in python3
		-Modified emceeutils.lnlhood to sum float values
...
class Emceebones(object):
    ...
    def __init__(self,data,infodata,model,nwalkers,nsamp,threads,outsave,optim,effnhi,restartchn):
        ...
        effnhi
        restartchn: string
          File to use for restarting chains
        ...
        self.restartchn=restartchn

        #to trigger correct behaviour at restart, switch to optim=guess
        if(self.restartchn):
            print('For restart, switch to GUESS optimization')
            self.optim='guess'
    ...
    def loadmodel(self, data, model):
        ...
        modl=h5py.File(model)

        #unpack axis tag, axis value, grid column, grid ions
        #NOTE: mod_axistag is ordered to match axes of model grid 
        #(this is hardcoded to match hdf5 file axis order)

        string=''.join(modl['parameters']['axistags'][()])
        translation_table = dict.fromkeys(map(ord, "[]'"), None)
        string = string.translate(translation_table)
        self.mod_axistag=string.split(', ')
        print('Axis of grid are ', self.mod_axistag)
        #self.mod_axistag=list(np.array(modl['parameters/axistags']).astype(str))
      

        #Identify the name of the redshift and column density axis tags in the model grid file.
        #Accepts either red/redshift or col/column; otherwise throws an error
        self.redkey='red'
        if 'red' not in self.mod_axistag:
            if 'redshift' in self.mod_axistag:
                self.redkey='redshift'
            else: 
                raise ValueError("Redshift axistags name within model grid is incorrect. Must be 'red' or 'redshift'.")

        self.colkey='col'
        if 'col' not in self.mod_axistag:
            if 'column' in self.mod_axistag:
                self.colkey='column'
            else: 
                raise ValueError("Column density axistags name within model grid is incorrect. Must be 'col' or 'column'.")
        ...
        for tt in self.mod_axistag:
            self.nmodels=self.nmodels*np.array(modl['parameters/%s'%tt]).size
            #append axis value in a list
            self.mod_axisval.append(np.array(modl['parameters/%s'%tt]))
        ...
        for obs in data:
            #Get element name and integer ionization
            elem=obs[0][0]
            if obs[0][1].isupper():
                nion=roman.fromRoman(obs[0][1:])
            else:
                elem=obs[0][0:2]
                nion=roman.fromRoman(obs[0][2:])
            ion=''
            if nion-1>0:ion+='+'
            if nion-1>1:ion+='%.0f'%(nion-1)
            ...
            if elem+ion in modl['ioncolumns'].keys():
                ...
                #Converting model columns to log space
                self.mod_colm.append(np.log10(np.array(modl['ioncolumns/%s'%(elem+ion)])))
        ...
        if(self.effnhi):
            ...
            #self.nhigrid=np.array(modl['parameters/column'])
            self.nhigrid=np.log10(np.array(modl['ioncolumns/H']))
    ...
    def plotinfo(self, sampler, use_pkl=False):
        ...
        if use_pkl:
            ...
            with h5py.File(self.outsave+'/'+self.info['name']+'_emcee.hd5', 'w') as f:
                ...
                mcmc_dict = dict(nwalkers=int(self.nwalkers), nsamp=int(self.nsamp),
                                 nburn=int(self.burn), nthread=int(self.threads))
                out_group.attrs['mcmc'] = str(json.dumps(mcmc_dict))
    ...
    def setup_emc(self):
        ...
        emc.redkey=self.redkey
        emc.colkey=self.colkey
    ...
    def __call__(self):
        ...
        #param=[3.226,17.7,-2,-2]
        #zzz=np.arange(17,18.5,0.05)
        #self.info['hiflag']=0
        #self.info['eNHI']=0.2
        #val=[]
        #for z in zzz:
        ...
        #Start by assigning initial guesses that will be modified according to different optimization choices next
        initguess=[]
        
        #if needed open hdf5 for generating initial guesses on previous output
        if(self.restartchn):
            restdata=h5py.File(self.restartchn)
            
        for pp in range(self.ndim):
            tag=self.mod_axistag[pp]
            #xx in tag syntax should catch different ways of calling axis
            if self.colkey in tag: 
                #Handle special case column density
                initguess.append(self.info['NHI'])
            elif self.redkey in tag:
                ...
                if self.optim in ['guess', 'guess_NHI']:
                    if(self.restartchn):
                        #use previous otput for restart
                        restdata=h5py.File(self.restartchn)
                        #pick median of results as initial guess
                        initguess.append(restdata['outputs']['results'][3][pp])
                    else:
                        #use provided values as guesses
                        if 'met' in tag:
                    ...
                    #if not specified pick middle of axis
                    initguess.append(np.median(self.mod_axisval[pp]))
        
            #close hdf5 if previously open
            if(self.restartchn):
                restdata.close()
        ...
        if self.optim in ['guess', 'guess_NHI']:
            ...
            #now tweak if requiring special NHI case
        ...
        print("[good range 0.25-0.5. Low/high is bad!]")
        ...
        self.final={'ions':self.mod_colm_tag,'percent':percentile,'results':results,'acceptance':accept,
                    ...
                    'data':self.data,'info':{},'effNHI':best_fit_nhi}
        #ions,tags,data,info,effNHI
        #COnvert all strings in self.final to numpy strings for hdf5
        self.final['ions']=np.string_(self.final['ions'])
        self.final['tags']=np.string_(self.final['tags'])
        for ii,obs in enumerate(self.final['data']):
            self.final['data'][ii]=(np.string_(obs[0]),obs[1],obs[2],obs[3])
        for key in self.info.keys():
            val=self.info[key]
            if type(val)==str: val=np.string_(val)
            self.final['info'][np.string_(key)]=val
...
class Emceeutils():
    ...
    def lnprior(self, param):
        ...
        if (inside):
            ...
            #start with redshift
            zindex=self.mod_axistag.index(self.redkey)
            ...
            nhiindex=self.mod_axistag.index(self.colkey)
    ...
    def test_interpolators(self):
        ...
        int_dens=np.arange(-4.5,1.25,0.1)
        int_colm=np.arange(16.0,20.75,0.1)
        int_red=np.arange(2.0,6.25,0.1)
        int_met=np.arange(-4.5,1.25,0.1)


        #loop over ions
        for ii in range(self.nions):

            print('Check interpolator for ion', ii)
            ...
            interp_1=np.zeros((int_red.size,int_colm.size))
            xx=0
            for cc in int_red:
                yy=0
                for rr in int_colm:
            ...
            interp_2=np.zeros((int_colm.size,int_dens.size))
            xx=0
            for cc in int_colm:
                yy=0
                for rr in int_dens:
            ...
            interp_3=np.zeros((int_dens.size,int_met.size))
            xx=0
            for cc in int_dens:
                yy=0
                for rr in int_met:
            ...
            interp_4=np.zeros((int_red.size,int_met.size))
            xx=0
            for cc in int_red:
                yy=0
                for rr in int_met:
...
def mcmc_ions(data,infodata,model,nwalkers=500,nsamp=250,threads=12,
              outsave='emceeout', optim=False, effnhi=True, testing=False,restartchn=None):
    ...
    restartchn: use a previous run output to initialize the chains 
    ...
    mcmc=Emceebones(data,infodata,model,nwalkers,
                    nsamp,threads,outsave,optim,effnhi,restartchn)
