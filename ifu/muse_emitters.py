...
Depend on public code SHINE (https://doi.org/10.5281/zenodo.14710519) or proprietary code [cubex] 
...
def independent_SNR_fast_SHINE(catalog, covariance, segmap, cube, var, cube_med=None, cube_odd=None, cube_even=None, 
    ...
    catalog       -> source catalog from SHINE
    ...
    for j in range(len(catalog)):
        ...
        #here is ok without any -1. NB: SHINE catalogue starts from 0. You must not use the correction -1 otherwise the first sources will have z1=-1 (it gives wrong results).
...
def make_images_fast_SHINE(cubelist, segcube, header, catentry, Id, outdir, outnamelist, padding=0):
    
    """
    Utility function that loops over cubes and generate cutouts (assuming the SHINE catalogue)
    ...
    for ii in range(len(cubelist)):
        ...
        thisima        = np.nansum(cubelist[ii]*pixmask, axis=0)   #take the cube and sum across z only the voxels associated with the ID in the segmap
        empty          = (thisima==0)                              #select where is 0
        thisima[empty] = cubelist[ii][zgeo,empty]                  #where is 0 add a the central layer associated to the ID
...
def finalcatalogue_SHINE(fcube, fcube_var, fsegmap, catpath, target_z=None, rest_line=None, vel_cut=None,
                   cov_poly=None, output_dir='./', fcube_median=None, fcube_odd=None,
    ...
    Function that processes the SHINE catalogue to: 
    ...
    fcube            -> cube used for SHINE extraction
    fcube_var        -> var cube for SHINE extraction
    fsegmap          -> segmentation map for SHINE extraction
    catpath          -> catalogue path produced by SHINE 
    ...
    fsource_img      -> [optional] if set to aperture image (and marzred set), exclude emitters projected against 
    ...
    cat_name=output_dir+catname.split(".fits")[0]+"_select_SNR.fits"
    ...
    if os.path.isfile(cat_name):
        ...
        print(f'Reading the catalogue in: {catpath} \n')
        ...
        if cov_poly is None:
            ...
            size       = np.sqrt(catalog['Nspat'])*0.2   #projected area in arcsecs
            ...
            for ii in range(len(catalog)):
                try:
                    #firstly try with flux weighted lambda
                    okind = np.where(catalog['LambdaFL'][ii]>cov_poly[:,0])[0][-1]
                except:

                    try:
                        #then try with geometric lambda
                        okind = np.where(catalog['Lambda'][ii]>cov_poly[:,0])[0][-1]
                    except:
                        okind = 0 
    ...
    print("Reading cubes")
    print(f"  -> filtered cube in {fcube}")
    ...
    print(f"  -> filtered variance in {fcube_var}")
    try:
       cube_var  = fits.open(fcube_var)[1].data
    except:
       cube_var  = fits.open(fcube_var)[0].data
        
    #reconstruct name of SHINE segmentation cube and open
    print(f"  -> segmentation map in {fsegmap}")
    segmap  = fits.open(fsegmap)[0].data
    
    if(fcube_odd):
        print(f" -> odd cube and var in {fcube_odd} and {fcube_odd_var}")
        try:
            try:
              cube_odd          = fits.open(fcube_odd)[1].data
            except:
              cube_odd          = fits.open(fcube_odd)[0].data
            try:   
              cube_odd_var      = fits.open(fcube_odd_var)[1].data
            except:
              cube_odd_var      = fits.open(fcube_odd_var)[0].data
        except:
    ...
    if(fcube_even):
        print(f" -> even cube and var in {fcube_even} and {fcube_even_var}")
        try:
            try:
              cube_even          = fits.open(fcube_even)[1].data
            except:
              cube_even          = fits.open(fcube_even)[0].data
            try:   
              cube_even_var      = fits.open(fcube_even_var)[1].data
            except:
              cube_even_var      = fits.open(fcube_even_var)[0].data
        except:
    ...
    if(fcube_median):
        print(f" -> median cube and var in {fcube_median} and {fcube_median_var}")
        try:
            try:
              cube_median       = fits.open(fcube_median)[1].data
            except:
              cube_median       = fits.open(fcube_median)[0].data
            try:
              cube_median_var   = fits.open(fcube_median_var)[1].data
            except:
              cube_median_var   = fits.open(fcube_median_var)[0].data
        except:
    ...
    if not os.path.isfile(cat_name):
        ...
        if(target_z is not None):
            veloff = velocityoffset(catalog['LambdaFL'], target_z, rest_line)
        ...
        if(derived):
            print("\nCalculating independent SNR and additional metrics for {} sources".format(len(catalog)))
            catalog = independent_SNR_fast_SHINE(catalog, covariance, segmap, cube, cube_var,\
        ...
        print("\nWriting the full catalog to disk")
        catalog.write(output_dir+catname.split(".fits")[0]+"_all_SNR.fits", format="fits", overwrite=True)
        ...
        catalog = catalog[select]
        print(f'\nSelecting the catalog to obtain only the sources above the minimum SN: {np.amin(SNcut)}')
        ...
        if (mask):
           ...
           masked  = msk[np.array(catalog['YcentFL'], dtype=int),np.array(catalog['XcentFL'], dtype=int)]  
        ...
        #Write full catalogue with SNR and derived quantities
        print('\nWriting the selected catalog to disk')
        catalog.write(output_dir+catname.split(".fits")[0]+"_select_SNR.fits", format="fits", overwrite=True)
    ...
    if(checkimg):
        ...
        if fcube_orig is not None:
            print("\nExtracting images and spectra for {} sources".format(len(catalog)))
        else:
            print("\nExtracting only images (not spectra) for {} sources".format(len(catalog)))
        ...
        for ii in range(startind,len(catalog)):
            ...
            if os.path.isdir(objdir):
                ...
                #make images (check which cubes are present)
                if cube_median and cube_odd and cube_even is not None:
                    hdulist=[cube, cube_median, cube_odd, cube_even]
                    outnamelist=['_mean', '_median', '_half1', '_half2']
                else:
                    hdulist=[cube]
                    outnamelist=['_mean']
                    make_images_fast_SHINE(hdulist, segmap, cubehdr, catalog[ii], objid, objdir, outnamelist, padding=50)
